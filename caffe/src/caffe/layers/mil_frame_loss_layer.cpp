...
//  LossLayer<Dtype>::LayerSetUp(bottom, top);
  ...
  this->curbatch = 0;
  this->predict_only = frame_param.predict_only();
  ...
  if(this->predict_only) LOG(INFO) << "PREDICT ONLY MODE";
  ...
  LOG(INFO) << this->verb_length.size();
  LOG(INFO) << this->verb_start.size();
  LOG(INFO) << this->arg_structure.size();
  LOG(INFO) << this->arg_verb.size();
  LOG(INFO) << this->arg_index.size();
...
void MILFrameLossLayer<Dtype>::Reshape(
 ...
 // LossLayer<Dtype>::Reshape(bottom, top);
  int batches = bottom[this->verb_index]->shape(0);
 
  LOG(INFO) << top.size() << " " << this->predict_only; 
  ...
  if( !this->predict_only){
    ...
    if( this->mode == 0 || this->mode == 1){
      top[0]->Reshape(batches,1,1,this->maxlabel*this->total_frames);
      top[1]->Reshape(batches,1,1,this->total_frames);
    }
    else if ( this->mode == 2){
      top[0]->Reshape(batches,this->total_syns,1,1);
    }
  }
  LOG(INFO) << "args " << this->total_args;
  LOG(INFO) << "batches " << batches;
  LOG(INFO) << "frames " << this->total_frames;
  LOG(INFO) << "max_value " << this->max_value;
...
void MILFrameLossLayer<Dtype>::Forward_cpu(
  ...
  if(!this->predict_only) bottom_label = bottom[this->label_index]->cpu_data();
