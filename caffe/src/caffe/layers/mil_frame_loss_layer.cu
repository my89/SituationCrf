...
template <typename Dtype>
__global__ void MILFrameRefScoreMarginal(const int nthreads, 
   const Dtype * marginal, Dtype* ref_scores,
   int i, int r, int a, int total_args, int total_bbs, int references, bool clear){ 
   CUDA_KERNEL_LOOP(index, nthreads) {
...
//     int value_index = i*total_bbs*length + b*length + gold_label;
     int marginal_index = i*total_bbs*total_args + a*total_bbs + b;
     ref_scores[ref_offset] = (clear ? 0 :  ref_scores[ref_offset]) + marginal[marginal_index];
  } 
}
...
template <typename Dtype>
__global__ void MILFrameFillVerbMax1BB(const int nthreads, 
  const Dtype* verb_potential, const Dtype* norm,  Dtype* output_scores, Dtype* output_structure,
  int total_frames, int maxlabel){
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    Dtype score = verb_potential[i*total_frames+v]; 
 
    output_scores[score_offset] = score - norm[i];
    output_structure[structure_offset] = v;
  }
}

template <typename Dtype>
__global__ void MILFrameFillArgMax1BB(const int nthreads, 
  const Dtype * arg_potential, Dtype* output_scores, Dtype* output_structure,
  int length, int frame, int a, int maxlabel, int total_frames){
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    int score_offset = i*total_frames + frame;
    int structure_offset = i*total_frames*maxlabel + frame*maxlabel + a + 1;

    Dtype mv = arg_potential[offset];
    Dtype mi = 0;
    for(int v = 1; v < length; v++){
      Dtype val = arg_potential[offset + v];
      if( val > mv){
        mv = val;
        mi = v;
      }
    ...
    output_scores[score_offset] += mv;
    output_structure[structure_offset] = mi;
  }
}
...
__global__ void MILArgBackward(const int nthreads, 
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    for(int r = 0; r < references; r++){
      ...
      //if( scalar > 1 ) scalar = 1; 
      Dtype x = 0.f; 
      if ( f == gold_verb && gold_value == -2) x = exp(value_score - _arg_marginal);
      else if(f == gold_verb && v == gold_value) x = 1.0f;
      else x = 0;
      g+= scalar * ( x  - expected);
...
void MILFrameLossLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int batch_size = bottom[this->verb_index]->shape(0);

  LOG(INFO) << "MIL START FORWARD " << batch_size << " " << total_bbs << " predict only=" << this->predict_only << " size=" << top.size();
  LOG(INFO) << bottom[this->verb_index]->shape(1); 
  ...
  CUDA_POST_KERNEL_CHECK;
  LOG(INFO) << "norm done";
  if(!this->predict_only){
  LOG(INFO) << "in predict only";
  ...
  for(int i = 0; i < batch_size; i++){
    for(int r = 0; r < references; r++){
      ...
      for(int a = 0; a < vlength; a++){        
          ...
          if( gold_value > arg_length) LOG(INFO) << "ARG INDEX ERROR!!!!!!" << gold_verb << " " << a << " " << gold_value;
	  if( gold_value == -1) LOG(INFO) << "ARG == -1!!!" ;
	  if (gold_value == -2){
            MILFrameRefScoreMarginal<Dtype><<<CAFFE_GET_BLOCKS(r_jobs), CAFFE_CUDA_NUM_THREADS>>>(
              r_jobs, arg_marginal.gpu_data(), ref_scores.mutable_gpu_data(),
              i, r, arg_index, total_args, total_bbs, references, false);
            CUDA_POST_KERNEL_CHECK;
	  }
	  else{
            MILFrameRefScore<Dtype><<<CAFFE_GET_BLOCKS(r_jobs), CAFFE_CUDA_NUM_THREADS>>>(
  ...
  if(this->mode == 1) this->PredictMaxFrameGPU(bottom,top);
  LOG(INFO) << "FORWARD DONE";
}


template <typename Dtype> 
void MILFrameLossLayer<Dtype>::PredictMaxFrameGPU(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top){  
  ...
  MILFrameFillVerbMax1BB<Dtype><<<CAFFE_GET_BLOCKS(v_jobs), CAFFE_CUDA_NUM_THREADS>>>( v_jobs,
    bottom[verb_index]->gpu_data(), norms.gpu_data(),  top[frame_index]->mutable_gpu_data(), top[score_index]->mutable_gpu_data(), 
    total_frames, maxlabel);
  CUDA_POST_KERNEL_CHECK;
  ...
  for(int f = 0; f < total_frames; f++){
      ...
      for( int a = 0; a < total_arg; a++){
        int arg_index = arg_offset + a;
        int length = arg_structure[arg_index];     
        int s_jobs = batch_size;  
        MILFrameFillArgMax1BB<Dtype><<<CAFFE_GET_BLOCKS(s_jobs), CAFFE_CUDA_NUM_THREADS>>>(s_jobs,
          bottom[arg_index]->gpu_data(), top[frame_index]->mutable_gpu_data(), top[score_index]->mutable_gpu_data(), 
          length, f, a, maxlabel, total_frames);
        CUDA_POST_KERNEL_CHECK;
     }
  }
}
...
template void MILFrameLossLayer<float>::PredictMaxFrameGPU( \
      const std::vector<Blob<float>*>& bottom, \
      const std::vector<Blob<float>*>& top); \
template void MILFrameLossLayer<double>::PredictMaxFrameGPU( \
      const std::vector<Blob<double>*>& bottom, \
      const std::vector<Blob<double>*>& top);
