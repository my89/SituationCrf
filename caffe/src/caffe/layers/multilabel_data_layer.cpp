...
void MultiLabelImageDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  this->dropout = image_data_param.label_dropout();
  this->ignore_zero = image_data_param.ignore_zero();    
...
void MultiLabelImageDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  for (int item_id = 0; item_id < batch_size; ++item_id) {
    ...
    //dropout by position, not individually by label
    for( int i = 0; i < max_labels; i++){
      if(this->dropout > 0){
          if(this->ignore_zero && i == 0) continue;
          float r = static_cast <float> (rand()) / static_cast <float> (RAND_MAX);
          if( r <= this->dropout){
            for( int k = 0; k < references; k++){
               prefetch_label[copy_base + k*max_labels + i] = -2;
            }
          }
        }
    }
