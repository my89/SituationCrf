...
#include "caffe/layers/structure_frame_accuracy_layer.hpp"
...
void StructureFrameAccuracyLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < batch_size; i++){    
	...
	int guess_offset = output_offset + _scores[k].second*this->maxlabel;
        ...
        outfile << buffer.str() << "\t" << _scores[k].first << "\n"; 
      }
    }
    if(found) verb_score++;
    int total_args = 0;
    ...
    for(int j = 1; j < this->maxlabel; j++){
      ...
      for(int r = 0; r < this->references; r++){
        if(bottom_refs[ref_offset+r*this->maxlabel+j] == -2){
	  arg_count --;
	  total_args --;
	  break;
	}
