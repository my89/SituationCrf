...
void MILFrameIndLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
//  LossLayer<Dtype>::LayerSetUp(bottom, top);
  ...
  bias_index = verb_curr+1;
  label_index = verb_curr+2;
...
template <typename Dtype>
void MILFrameIndLossLayer<Dtype>::toBlob(vector<int> in , Blob<Dtype> * out){   
...
template <typename Dtype>
void MILFrameIndLossLayer<Dtype>::Reshape(
 ...
 // LossLayer<Dtype>::Reshape(bottom, top);
  ...
  if( this->mode == 0){
    top[4]->Reshape(batches,1,1,this->maxlabel*this->total_frames);
    top[5]->Reshape(batches,1,1,this->total_frames);
  }
  else if ( this->mode == 1){
    top[4]->Reshape(batches,this->total_syns,1,1);
  }
  ...
  this->contradiction_scores.Reshape(batches,this->total_frames, 1, 1);
  this->total_scores.Reshape(batches, 1 , 1, 1);
  this->total_negative_scores.Reshape(batches, 1 , 1 , 1);
}

template <typename Dtype> 
Dtype MILFrameIndLossLayer<Dtype>::log_sum_exp(int length, int offset, int increment, const Dtype* data){
  int kmin = offset;
...
template <typename Dtype>
void MILFrameIndLossLayer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) { 
/*
...
template <typename Dtype> 
void MILFrameIndLossLayer<Dtype>::PredictMarginalFrame(const vector<Blob<Dtype>*>& top, const Dtype* verb, std::vector<const Dtype*> args, const Dtype* label){
...
template <typename Dtype>
void MILFrameIndLossLayer<Dtype>::PredictMarginalObject(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top)
{
...
template <typename Dtype>
void MILFrameIndLossLayer<Dtype>::Backward_cpu(
...
INSTANTIATE_CLASS(MILFrameIndLossLayer);
REGISTER_LAYER_CLASS(MILFrameIndLoss);
