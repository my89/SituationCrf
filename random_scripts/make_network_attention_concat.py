...
for (verbid, args) in verb_def.items():
	for (argid, arglength) in args.items():
		...
		split_top += '\n  top : "rep-' + basename+'"'
		split_top_global += '\n top: "value-global-' + basename +'"'	
		total_arg_lengths += int(arglength)
		if k < total_args: 
			split_index += '    slice_point: ' + str(k*512) + "\n";
                	split_index_global += '    slice_point: ' + str(total_arg_lengths) + "\n"     
			k+=1
...
#global_inner = global_inner.replace("${OUTDIM}", str(total_arg_lengths))
#do the splitting
...
for (verbid, args) in verb_def.items():
        ...
        verb_rep = verb_rep.replace("${OUTDIM}", str(64))
        relu = relu_template.replace("${NAME}", '  name: "relu-'+verb_name+ '"')
        relu = relu.replace("${TOP}", '  top:"relu-'+verb_name+'"')
...
#		relu = relu.replace("${BOTTOM}", '  bottom: "embed-'+basename+'"')
		...
		sumv = sumv.replace("${TOP}", 'top: "concat-' + basename + '"')
		sumv = sumv.replace("${BOTTOM1}", 'bottom: "dropout-' + basename + '"')
		sumv = sumv.replace("${BOTTOM2}", 'bottom: "dropout-' + verb_name + '"')

		value = inner_product_template2.replace("${NAME}", '  name: "value-' + basename + '"')
		value = value.replace("${TOP}", '  top: "value-' + basename + '"')
		value = value.replace ("${BOTTOM}", '  bottom: "concat-'+ basename+ '"')
