...
sum_template = open("sum.template").read()
concat_template = open("concat.template").read()
...
for (verbid, args) in verb_def.items():
	split_top_global += '\n  top : "embed_verb-' + id_verb[verbid]+'"'
        if v < len(verb_def):
		split_index_global += '    slice_point: ' + str(int(in_arg_rep_size)*v) + "\n"     	
		v+=1

	for (argid, arglength) in args.items():
		basename = id_verb[verbid] + '-' + verb_id_arg[verbid][argid]
		split_top += '\n  top : "rep-' + basename+'"'
		total_arg_lengths += int(arglength)
		if k < total_args: 
			split_index += '    slice_point: ' + str(k*512) + "\n";
			k+=1
...
#make the inner product for values
global_inner = inner_product_template2.replace("${NAME}", ' name: "value-global"')
global_inner = global_inner.replace("${TOP}", 'top:"value-global"')
global_inner = global_inner.replace("${BOTTOM}", split_bottom)
global_inner = global_inner.replace("${OUTDIM}", str(int(in_arg_rep_size)*len(verb_def)))
#do the splitting
sl2 = slice_template.replace("${BOTTOM}", '  bottom:"value-global"');
sl2 = sl2.replace("${TOP}" , split_top_global);
sl2 = sl2.replace("${SLICE}", split_index_global); 

output = output + "\n" + attention + "\n" + attend + "\n" + global_inner + "\n" + sl + "\n" + sl2 + "\n"        
#create three layers inner_product to arg embedding, relu, inner_product to arglength
for (verbid, args) in verb_def.items():
        verb_name = id_verb[verbid]

        #verb_rep = inner_product_template.replace("${NAME}",'   name:"rep_verb-'+verb_name+'"')
        #verb_rep = verb_rep.replace("${BOTTOM}", split_bottom)
        #verb_rep = verb_rep.replace("${TOP}", '   top:"rep_verb-'+verb_name+'"')
        #verb_rep = verb_rep.replace("${OUTDIM}", in_arg_rep_size)
        #relu = relu_template.replace("${NAME}", '  name: "relu_verb-'+verb_name+ '"')
        #relu = relu.replace("${TOP}", '  top:"relu_verb-'+verb_name+'"')
        #relu = relu.replace("${BOTTOM}", '  bottom:"rep_verb-'+verb_name+'"')            
	#dropout = dropout_template.replace("${NAME}", '  name: "dropout-'+verb_name+'"')
	#dropout = dropout.replace("${TOP}", '  top: "dropout-'+verb_name+'"')
	#dropout = dropout.replace("${BOTTOM}", '  bottom: "relu-'+verb_name+'"')               
...
#                attend = attend.replace("${BOTTOM2}", '  bottom: "softmax-'+basename+'"');   
 #               attend = attend.replace("${TOP}", '  top: "attend-rep-'+basename+'"');
		...
		dropout = dropout_template.replace("${NAME}", '  name: "dropout-'+basename+'"')
		dropout = dropout.replace("${TOP}", '  top: "rep-'+basename+'"')
		dropout = dropout.replace("${BOTTOM}", '  bottom: "rep-'+basename+'"')               
		...
		relu = relu.replace("${TOP}", '  top:"embed-'+basename+ '"')
		relu = relu.replace("${BOTTOM}", '  bottom: "embed-'+basename+'"')

		sumv = concat_template.replace("${NAME}", 'name: "concat-' + basename + '"')
		sumv = sumv.replace("${TOP}", 'top: "embed2-' + basename + '"')
		sumv = sumv.replace("${BOTTOM1}", 'bottom: "embed-' + basename + '"')
		sumv = sumv.replace("${BOTTOM2}", 'bottom: "embed_verb-' + verb_name + '"')

		value = inner_product_template2.replace("${NAME}", '  name: "value-' + basename + '"')
		value = value.replace("${TOP}", '  top: "value-' + basename + '"')
		value = value.replace ("${BOTTOM}", '  bottom: "embed2-'+ basename+ '"')
