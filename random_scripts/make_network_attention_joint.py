...
for (verbid, args) in verb_def.items():
	...
	verbname = 'verb_' + id_verb[verbid];
        dropout = dropout_template.replace("${NAME}", '  name: "dropout-'+verbname+'"')
	dropout = dropout.replace("${TOP}", '  top: "rep-'+verbname+'"')
	dropout = dropout.replace("${BOTTOM}", '  bottom: "rep-'+verbname+'"')               

        embed = inner_product_template.replace("${NAME}", '  name: "embed-' + verbname + '"')
	embed = embed.replace("${TOP}", '  top: "embed-'+verbname+'"')#
	embed = embed.replace("${BOTTOM}", '  bottom: "rep-'+verbname+'"')
	embed = embed.replace("${OUTDIM}" ,in_arg_rep_size)
		
        relu = relu_template.replace("${NAME}", '  name: "relu-'+verbname+ '"')
	relu = relu.replace("${TOP}", '  top:"embed-'+verbname+ '"')
	relu = relu.replace("${BOTTOM}", '  bottom: "embed-'+verbname+'"')

        dropout2 = dropout_template.replace("${NAME}", '  name: "dropout2-'+verbname+'"')
	dropout2 = dropout2.replace("${TOP}", '  top: "embed-'+verbname+'"')
	dropout2 = dropout2.replace("${BOTTOM}", '  bottom: "embed-'+verbname+'"')               
...
#                attend = attend.replace("${BOTTOM2}", '  bottom: "softmax-'+basename+'"');   
 #               attend = attend.replace("${TOP}", '  top: "attend-rep-'+basename+'"');
		...
		value = value.replace ("${BOTTOM}", '  bottom: "embed-'+ verbname+ '"')
