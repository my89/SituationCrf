...
for (verbid, args) in verb_def.items():
	for (argid, arglength) in args.items():
		total_args_length += int(arglength)
		basename = id_verb[verbid] + '-' + verb_id_arg[verbid][argid]
		split_top += '\n  top : "value-' + basename+'"'
		if k < total_args: 
			split_index += '    slice_point: ' + str(total_args_length) + "\n";
			k+=1
...
global_inner = inner_product_template2.replace("${NAME}", '  name: "arg-inner"')
global_inner = global_inner.replace("${TOP}", '  top: "arg-inner"')#
global_inner = global_inner.replace("${BOTTOM}", split_bottom)
global_inner = global_inner.replace("${OUTDIM}" , str(total_args_length))


#we will introduce a slicing layer here. 
sl = slice_template.replace("${BOTTOM}", '  bottom:"arg-inner"');
...
#		dropout2 = dropout_template.replace("${NAME}", '  name: "dropout2-'+basename+'"')
#		dropout2 = dropout2.replace("${TOP}", '  top: "embed-'+basename+'"')
#		dropout2 = dropout2.replace("${BOTTOM}", '  bottom: "embed-'+basename+'"')               


#		value = inner_product_template2.replace("${NAME}", '  name: "value-' + basename + '"')
#		value = value.replace("${TOP}", '  top: "value-' + basename + '"')
#		value = value.replace ("${BOTTOM}", '  bottom: "embed-'+ basename+ '"')
