...
in_label_blob_name = sys.argv[4]
in_structure_def = sys.argv[5]
in_arg_rep_size = sys.argv[6]
in_frame_rep_size = sys.argv[7]
out_network = sys.argv[8]
...
for (verbid, args) in verb_def.items():
	for (argid, arglength) in args.items():
		...
		embed = embed.replace("${OUTDIM}" ,str(min(int(in_arg_rep_size), max(1, int(float(arglength)/4)))))
...
#		dropout = dropout.replace("${BOTTOM}", '  bottom: "relu-'+basename+'"')
		value = inner_product_template.replace("${NAME}", '  name: "value-' + basename + '"')
		value = value.replace("${TOP}", '  top: "value-' + basename + '"')
		value = value.replace ("${BOTTOM}", '  bottom: "relu-'+ basename+ '"')
...
value = value.replace("${BOTTOM}", '  bottom: "relu-frame"')
